...
“Push to git” authorizes only the current branch. A request to open a PR does not authorize merge. “Merge this,” “land this,” or “merge this to main” authorizes one complete Git closeout: push the scoped branch, open or update a ready PR, wait for required CI and blocking review state, merge, verify the accepted revision on the remote default branch, run established revision-attributable post-merge monitoring when available, fast-forward the local default branch, delete the remote branch, and safely remove the isolated worktree and local branch when no unique work remains. Stop cleanup and preserve state on dirty or untracked work, branch drift, unique commits, another-worktree use, or failed monitoring. Generic merge authority does not authorize local installation or task archival. Deployment, production changes, destructive actions, migrations, credentials, paid actions, and rollback remain limited to the accepted product and available authority.
