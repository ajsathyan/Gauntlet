...
| PRD execution contract | Add + move | A product PRD may hold many Epics; one frozen launch creates one visible task and one Execution Run per target Epic. | `docs/prd-execution.md`, `docs/local-documentation.md`, planner and `implement-prd` |
...
| Ticket scheduling | Add + move | Use a critical-path ready queue, affinity, incremental integration, optional Cohorts only for shared invariants, direct parent proof, and one final Epic verification. | `docs/prd-execution.md` and planner |
