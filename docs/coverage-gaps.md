...
- The same class of issue appears across multiple completed changes.
...
At the end of the work, mention only new or updated gap IDs and why they matter. Put them at the end of the final response using this shape:
...
Remove a `GAP-###` entry from this file when the accepted destination exists and is covered by proof. Do not keep resolved gaps here as historical archive; Git history is the archive.
...
- A durable change or decision records that the gap was resolved or removed.

If a gap is only partially addressed, keep it pending and narrow its wording to the remaining missing guidance.

## Promotion Scanner

When `promotion-scanner` finds high-value repeated work, add or update a `GAP-###` only for Gauntlet-general missing guidance. Repo-specific promotion candidates should become repo code, repo test, repo docs, or issue follow-up.
