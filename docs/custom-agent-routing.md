...
Gauntlet selects an execution boundary from explicit Ticket fields. Selection is implemented by `scripts/route-codex-agent.py`; it does not call an LLM router. Most delegated work returns a custom-agent profile that the parent passes as `agent_name` in native `spawn_agent`. Security proof returns the dedicated `security-review` Codex CLI runner instead. The parent remains responsible for ticket compilation, integration, the acceptance oracle, and every merge, release, deployment, production, or rollback decision.
...
1. If `work_class = verification` and `proof = security`, return `status = codex-cli` and `runner = security-review`. Create this separate review only when a consequential trust, credential, permission, paid/destructive authority, migration, or production boundary triggers it; ordinary implementation uses direct parent verification. Execute the bounded Ticket with:
   ...
   The runner invokes non-interactive, ephemeral `codex exec` with an explicit `--sandbox read-only`, `approval_policy="never"`, high reasoning effort, ignored user configuration, and native multi-agent dispatch disabled. The output path is optional, but when used it must be outside the reviewed workspace.
...
- Route newly discovered security-sensitive surfaces to the `security-review` Codex CLI runner for independent read-only review.
...
Reconciliation classifies `profile_substitution`, `authority_substitution`, `model_substitution`, `reasoning_substitution`, `security_sandbox_violation`, and `missing_start_metadata` for native subagents and historical reviewer records. Its circuit is scoped to the native Codex version and requested profile. The first mismatch opens that circuit for later affected native dispatches; it does not stop an already-running ordinary child when the substituted profile has equivalent authority. Consequential substitutions, authority changes, and missing required start metadata return a fail-closed result. Never silently substitute another profile.
...
An open circuit returns exit status `3` before a new native spawn. A healthy native route still requires exactly one `spawn_agent` call and no model handshake. A `codex-cli` security route never enters native profile reconciliation or its circuit; `security-review.py` establishes the sandbox directly on every invocation. The router only reads compact circuit state; audit synchronization and rollout analytics stay outside the scheduling critical section.
...
The security runner does not depend on a custom-agent default or user configuration for authority. Its command construction is regression-tested to require `--sandbox read-only`, refuse review output inside the target workspace, and disable nested native dispatch.
