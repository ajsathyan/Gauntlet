...
SCRIPT = Path(__file__).with_name("eval-harness.py")
...
TASK_SCRIPT = Path(__file__).with_name("eval-task.py")
...
class HarnessFixture:
    def __init__(self, root: Path) -> None:
        ...
        self.cli.write_text(FAKE_CLI); self.cli.chmod(0o755)
        self.scorer.write_text(SCORER); self.scorer.chmod(0o755)
        self.start = root / "start"; self.start.mkdir(); (self.start / "README.md").write_text("fixture")
        self.state_digest = eval_harness.tree_digest(self.start)
        self.prompt = root / "prompt.md"; self.prompt.write_text("implement the development fixture")
...
class EvalHarnessTests(unittest.TestCase):
    ...
    def test_manifests_build_native_noninteractive_commands_without_prompt_argv(self) -> None:
        ...
        self.assertIn("exec", command); self.assertIn("--json", command); self.assertIn("--ephemeral", command)
        ...
        self.assertIn("-p", command); self.assertIn("stream-json", command); self.assertNotIn("--max-turns", command)
        self.assertNotIn("implement the development fixture", command)
    ...
    def test_manifest_rejects_inline_secrets_and_unresolved_models(self) -> None:
        ...
        manifest = self.fixture.manifest(); manifest["model"] = "SET_ME"
        ...
        with self.assertRaisesRegex(eval_harness.HarnessError, "environment keys must match"):
            eval_harness.validate_manifest(manifest)
        manifest = self.fixture.manifest("claude-code"); manifest["reasoning_effort"] = "high"
    ...
    def test_adapter_rejects_selective_rerun_and_wrong_starting_state(self) -> None:
        ...
        with self.assertRaisesRegex(eval_harness.HarnessError, "selective rerun"):
            eval_harness.execute_adapter(manifest, tasks, self.fixture.workspaces, request, 5)
        changed = self.fixture.request("execution-two"); changed["state_digest"] = "sha256:" + "2" * 64
    ...
    def test_empty_registry_is_ready_but_core_identity_is_rejected(self) -> None:
        ...
        registry = self.fixture.tasks(); registry["tasks"]["CORE-01"] = registry["tasks"].pop("dev-harness")
    ...
    def test_allowlisted_artifact_symlink_is_not_accepted_as_evidence(self) -> None:
        candidate = self.fixture.root / "candidate"; candidate.mkdir()
        target = self.fixture.root / "outside.txt"; target.write_text("outside")
    ...
    def test_aa_equivalence_is_same_harness_same_model_only(self) -> None:
        left = observations(); right = copy.deepcopy(left)
        ...
        self.assertEqual(result["status"], "fail"); self.assertEqual(result["mismatches"], ["permission"])
        cross_model = observations(); cross_model["model"] = "other-model"
        with self.assertRaisesRegex(eval_harness.HarnessError, "separate study blocks"):
            eval_harness.aa_compare(left, cross_model)
        cross_harness = observations(); cross_harness["harness"] = "claude-code"
        with self.assertRaisesRegex(eval_harness.HarnessError, "separate study blocks"):
            eval_harness.aa_compare(left, cross_harness)
        cross_version = observations(); cross_version["harness_version"] = "9.9.1"
        with self.assertRaisesRegex(eval_harness.HarnessError, "same harness_version"):
            eval_harness.aa_compare(left, cross_version)
        cross_resources = observations(); cross_resources["resource_profile"] = "large"
