...
from gauntletlib.core.fsio import atomic_write_synced_text as atomic_text
...
def normalize_scope_areas(source_areas: Any) -> list[str]:
    scope_out: list[str] = []
...
def normalize_cohorts(cohorts: dict[str, Any]) -> dict[str, Any]:
...
def normalize_planned_checks(planned_checks: Any, ticket_ids: set[str]) -> list[dict[str, Any]]:
...
def normalize_verification_and_review(
    verification_identity_raw: Any,
    review_raw_facts: Any,
    ...
    return verification_identity_out, review_out_facts


def normalize_review_units(
    review_raw: Any,
    ticket_out: list[dict[str, Any]],
) -> dict[str, Any]:
    ticket_ids = {item["id"] for item in ticket_out}
...
def normalize_shared_context(
    ...
    return {
        "cohorts": {
            key: require_string(cohort_context[key], f"shared context {key}")
            for key in sorted(cohort_context)
        },
...
def normalize_graph(raw: Any) -> dict[str, Any]:
    ...
    scope_out = normalize_scope_areas(source_areas)
    cohort_out = normalize_cohorts(cohorts)
    ticket_out = normalize_tickets(tickets, scope_out, cohort_out)
    ticket_ids = {item["id"] for item in ticket_out}
    planned_out = normalize_planned_checks(raw.get("planned_checks"), ticket_ids)
    verification_identity_out, review_out_facts = normalize_verification_and_review(
        raw.get("verification_identity"),
        raw.get("review"),
    )
    review_out = normalize_review_units(raw.get("review_units"), ticket_out)
    return {
        "cohorts": cohort_out,
        "scope_areas": scope_out,
        "shared_context": normalize_shared_context(shared, cohort_out),
...
def pending_review_unit_dependencies(
    manifest: dict[str, Any],
    unit: dict[str, Any],
) -> list[str]:
    return [
        dependency for dependency in unit["dependencies"]
        if REVIEW_UNIT_STATES.index(manifest["review_units"][dependency]["state"])
        < REVIEW_UNIT_STATES.index("merged")
    ]
...
def cmd_review_unit(args: argparse.Namespace) -> None:
    ...
    if action == "opened":
        ...
        if current_index < target_index:
            pending_dependencies = pending_review_unit_dependencies(manifest, unit)
...
def materialize_bundle(run: Path, manifest: dict[str, Any], ticket_id: str) -> dict[str, Any]:
    ...
    try:
        rendered = render_manifest(
            context_manifest,
            source_root=run,
            template_root=Path(__file__).resolve().parents[3] / "templates" / "generated-context",
        )
...
def reconcile_impact(
    ...
    lock: dict[str, Any],
    new_scope_hashes: dict[str, str],
) -> tuple[set[str], set[str], set[str], set[str]]:
    changed_scopes = {
        key for key in set(lock["scope_hashes"]) | set(new_scope_hashes)
        if lock["scope_hashes"].get(key) != new_scope_hashes.get(key)
    }
    impacted = {
        ticket_id for ticket_id, item in new_by_id.items()
        if changed_scopes.intersection(item["scope_area_ids"])
        or object_hash(item) != object_hash(old_by_id[ticket_id])
    }
    ...
    return changed_scopes, impacted, changed_cohort_contexts, impacted_units
...
def cmd_reconcile(args: argparse.Namespace) -> None:
    ...
    old_shared = old_graph["shared_context"]
    new_shared = graph["shared_context"]
    changed_scopes, impacted, changed_cohort_contexts, impacted_units = reconcile_impact(
        graph, old_graph, old_by_id, new_by_id, lock, new_scope_hashes,
    )
...
def parser(*, prog: str | None = None) -> argparse.ArgumentParser:
    root = argparse.ArgumentParser(prog=prog, description=__doc__)
...
def invoke(arguments: list[str], *, cwd: Path | str | None = None) -> ControllerResult:
    stdout = io.StringIO()
    stderr = io.StringIO()
    with _INVOKE_LOCK:
