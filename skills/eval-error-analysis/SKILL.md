...
- Use the `eval-synthetic-data` skill
...
Go back through all traces and apply binary labels (pass/fail) for each failure category. Each trace gets a column per category. Use whatever tool the user prefers — spreadsheet, annotation app (see `eval-review-interface`), or a simple script.
