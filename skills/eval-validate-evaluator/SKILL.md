---
name: eval-validate-evaluator
description: >
  Calibrate an LLM judge against human labels using data splits, TPR/TNR, and
  bias correction. Use after writing a judge prompt (eval-judge-prompt) when you
...
- A built LLM judge prompt (from `eval-judge-prompt`)
