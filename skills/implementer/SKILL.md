---
name: implementer
description: Use when executing a bounded Build workstream against an accepted design while preserving repo patterns, unrelated work, and meaningful proof.
---

# Implementer

Execute one bounded workstream from the accepted design. The design's `Acceptance` section remains the canonical Build Contract; a child assignment cannot narrow it.
...
- Native Codex state owns coordination. Child prompts contain only their accepted outcome slice and assignment, with disjoint ownership, state, and proof; otherwise work sequentially.
...
- The parent hands the exact integrated revision to Verify. A child receipt, green sensors, or this report cannot establish the accepted Build outcome.
