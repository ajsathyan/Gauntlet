...
- Findings:
  - P2: retry helper swallows the final error in one branch. Concrete risk: failed sync can appear successful. Suggested fix: rethrow the last error. Test gap: assert final-attempt rejection while a control proves an earlier successful retry still resolves; verify the test fails against the swallowing implementation.
