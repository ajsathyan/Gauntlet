...
Use the stable packet templates in [breakthrough-agent-packet.md](assets/breakthrough-agent-packet.md) and [observable-review-agent-packet.md](assets/observable-review-agent-packet.md). Render them with [render_agent_prompt.py](scripts/render_agent_prompt.py), preserving the complete template as the exact prompt prefix and appending canonical variable assignment JSON only at the end. Prefer artifact paths and hashes over copied evidence. Version and hash the populated packet and rendered prompt, then index the metadata receipt from `refactor-state.json`. Reuse one rendered breakthrough prompt unchanged for all three proposal agents.
