...
5. Run [measure_loc.py](../scripts/measure_loc.py) against the approved snapshot. Inspect and freeze its rules first. Add behavior-bearing declarative sources, such as Markdown skills, templates, or notebooks, when they execute or define product behavior; do not count ordinary documentation merely to enlarge the baseline. Use physical nonblank LOC as the canonical source measure, preserve the emitted rules hash and tree digest, and report production/test, generated, configuration, fixture, and migration categories separately. Record additional exclusions such as vendored or minified inputs. Treat a production/test reduction accompanied by growth in excluded categories as displaced complexity until reviewed; the comparator blocks that result.
