...
3. In each visible Epic task, read `source.snapshotPath` from the launch set and pass that immutable snapshot to `prd-run.py init --source ... --launch-set ...` with exactly one `--target` and that Epic's release stages. Never pass the mutable canonical PRD path. Freeze `single-final-pr` for a small reviewable Epic or `review-prs-plus-final` for a large tightly coupled Epic. Immediately after `epic-tasks record-run`, execute its `open_browser` action: read the private state file, construct the transient capability fragment, invoke or reuse the Codex in-app Browser, and let the client strip the fragment. Printing the action or URL is not execution. If Browser or server startup is unavailable, continue quietly; reconcile will recover it without blocking implementation.
4. Compile one deterministic Ticket Graph for the locked Epic. Copy the locked consequence trigger set exactly; graph inference may not omit or add triggers. Keep one implementation owner per Ticket. Cohorts are optional and exist only for a named shared invariant. Use direct parent verification by default; create independent verification Tickets only for a declared consequential boundary.
...
10. Generate schema 3.0 Project PR facts from controller state; do not author a project summary or Epic outcome artifact. Complete merge and applicable release stages, reconcile the exact projection into the canonical PRD/index, and let the product task start newly dependency-ready Epics. Keep the one launch-scoped progress supervisor alive through sibling failure; after every Epic is complete, stopped, or failed, run the controller's idempotent `progress-stop` during cleanup or archive.
