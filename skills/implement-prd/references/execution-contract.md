...
Each target Epic gets one visible implementation task and one Execution Run. The visible task carries only a compact launch envelope, never the Epic section. Before run creation, the Epic task executes the envelope's exact `bootstrap.argv` once; bootstrap verifies the launch coverage, task binding, immutable snapshot digest, complete accepted target, and locked Epic semantics, then returns the complete `epicSection` and `sourceSnapshot`. Failure stops execution, and no task-message fallback is valid. The Epic task passes that immutable snapshot to `prd-run.py init --source`; it never passes the mutable canonical PRD path and never creates another Epic task. A dependency blocks only the downstream Epic whose declared `merged`, `deployed`, or `productionProved` boundary is unsatisfied; unrelated Epics continue.
